...
public class AdditionSubtractionOptimiser {
    ...
    public static Expression optimise(Expression expression) {
        if (expression instanceof AdditionExpression)
            ...
            ((BinaryOperationExpression) expression).setLeft(optimise(((BinaryOperationExpression) expression).getLeft()));
            ((BinaryOperationExpression) expression).setRight(optimise(((BinaryOperationExpression) expression).getRight()));
        }
        return expression;
    }
    ...
    private static ArrayList<Term> getTerms(BinaryOperationExpression expression, boolean positive) {
        System.out.println(expression.getClass().getSimpleName()+" Left: "+expression.getLeft().getClass().getSimpleName()+" Right: "+expression.getRight().getClass().getSimpleName());
        ArrayList<Term> terms = new ArrayList<>();
        if ((expression.getLeft() instanceof AdditionExpression|| expression.getLeft() instanceof SubtractionExpression) && expression.getType()== expression.getLeft().getType()) {
        ...
        if ((expression.getRight() instanceof AdditionExpression || expression.getRight() instanceof SubtractionExpression) && expression.getType()== expression.getRight().getType()) {
        ...
        for (Term term : terms) {
            ...
            if (expression instanceof BooleanValue)
            {
                if (type==VariableType.INT)
                    sumInt=new IntValue(add(sumInt.getValue(), (((BooleanValue) expression).getValue() ? 1 : 0), positive));
                else if (type==VariableType.FLOAT)
                    sumFloat=new FloatValue(add(sumFloat.getValue(), (((BooleanValue) expression).getValue() ? 1 : 0), positive));
                else
